...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        else if (args.Length > 2 && args[0] == "-GenerateXML")
        {
            using (var scope = serviceProvider.CreateScope())
            {
                var serviceProviderScoped = scope.ServiceProvider;
                var xmlCreator = serviceProviderScoped.GetService<XmlCreator>();
                await xmlCreator.CopyDataBetweenDbContexts();
                await xmlCreator.PrepareData(args[2]);
        ...
        else
        {
            ...
            Console.WriteLine("  -GenerateXML [outputXmlFile] [outputErrorFile]     Generates an XML file with the current data.");
            Console.WriteLine();
